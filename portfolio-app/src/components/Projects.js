...
`;
function title() {
  return <h1 className="headers">My Projects</h1>;
}
      ...
      {data.map((repo) => {
        ...
        return (
          <ProjectCard key={repo.name}>
            ...
            <Container>
              <CommitLogo src={commitLogo} /> &ensp;{" "}
              {repo.commits > 0 ? repo.commits : "-"} &ensp; &ensp;
              <PeopleLogo src={contributorsLogo} /> &ensp;{" "}
              {repo.contributor_count > 0 ? repo.contributor_count : "-"}
            </Container>
            ...
            <ColorButton
              href={repo.link}
              color={colors[repo.langs[0]].color}
              text="Check it out!"
            />
            <br />
            <br />
